...
const UserSchema = z
  .object({
    ...
    name: z.string().min(1).max(100).meta({ description: "Display name", example: EXAMPLE_NAME }),
    status: z
      .enum(["Happy", "Sad"])
      .optional()
      .default("Happy")
...
app.openapi(
  createRoute({
    ...
    request: {
      ...
      query: z.object({
        name: z.string().optional().meta({ description: "Override display name", example: EXAMPLE_NAME }),
      }),
    },
    responses: {
